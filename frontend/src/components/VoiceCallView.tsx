...
interface Props {
  ...
  onChangeMicrophoneGain: (gainPercent: number) => void;
}
    ...
    <div className="flex-1 flex flex-col bg-bg-secondary min-w-0 h-full">
      ...
      {audioPanelOpen && (
        <section className="grid gap-4 px-5 py-4 border-b border-white/5 bg-bg-primary/40 animate-fade-in md:grid-cols-2" aria-label="Configurações de áudio">
          ...
          <label className="flex flex-col gap-1.5 text-xs text-text-muted">
            <span>Amplificação do microfone</span>
            <div className="flex items-center gap-3">
              <input
                ...
                onChange={(event) => onChangeMicrophoneGain(Number(event.target.value))}
                className="accent-brand flex-1"
                aria-label="Amplificação do microfone"
              />
              <span className="w-9 text-right tabular-nums text-text-primary">{microphoneGain}%</span>
            </div>
          </label>
          <label className="flex flex-col gap-1.5 text-xs text-text-muted">
